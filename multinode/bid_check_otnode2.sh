...
BIDS=$(docker logs otnode2 --since "$BID_CHECK_INTERVAL_DOCKER" | grep Accepting | wc -l)
#echo Bids: $BIDS

if [ $BIDS -eq 0 ]; then
  /root/OT-Settings/data/send2A.sh "Has not bid since $BID_CHECK_INTERVAL_DOCKER, restarting node"
  docker restart otnode2
fi

JOBS=$(docker logs otnode2 --since "$BID_CHECK_INTERVAL_DOCKER" | grep 've been chosen' | wc -l)
#echo Jobs: $JOBS

OFFER_ID=($(docker logs otnode2 --since "$BID_CHECK_INTERVAL_DOCKER" | grep 've been chosen' | grep -Eo '0x[a-z0-9]+'))
...
then
  ...
  do
    ...
    /root/OT-Settings/data/send2A.sh "Job awarded: $DAYS days at ${TOKEN_ARRAY[@]} TRAC"
  done
fi
