...
BIDS=$(docker logs otnode3 --since "$BID_CHECK_INTERVAL_DOCKER" | grep Accepting | wc -l)
#echo Bids: $BIDS
...
JOBS=$(docker logs otnode3 --since "$BID_CHECK_INTERVAL_DOCKER" | grep 've been chosen' | wc -l)
#echo Jobs: $JOBS

OFFER_ID=($(docker logs otnode3 --since "$BID_CHECK_INTERVAL_DOCKER" | grep 've been chosen' | grep -Eo '0x[a-z0-9]+'))
